...
def generator(config):
    ...
    if usePlanner:
        log.info("In this data we are using planner")
    ...
    while(1):
        ...
        while num_demo < config.num_demo_per_program and \
            ...
            try:
                ...
                kw, plan_and_program_differ = karel_util.generate_execution_example(s_gen, h, w, wall_prob, dsl, random_code, usePlanner)
                ...
                if len(kw.s_h) <= config.max_demo_length and len(kw.s_h) >= config.min_demo_length:
                    s_h_list.append(np.stack(kw.s_h, axis=0))
                    a_h_list.append(np.array(kw.a_h))

                    num_demo += 1
                    if plan_and_program_differ is True:
                        total_differs += 1
                    log.debug("Generated {} / {} demos for program {}".format(num_demo, config.num_demo_per_program, random_code))
        ...
        # progress bar
        count += 1
        log.info("{}: {}".format(count, random_code))
        ...
        if count >= num_total:
            ...
            if usePlanner:
                log.info("Total number of demos where planner actions and program actions differ is {}".format(total_differs))
            log.info("Total number of attempts to generate a demo was {}".format(total_num_attempts))
            return
